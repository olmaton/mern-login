...
controlador.sigin = async (req,res) =>{  
    try{  
        ...
        if(email_username||password){
            ...
            res.status(400).json({status:false,msg:"Datos incompletos",data: req.body})
        }
    }catch(e){
        console.log(e)
        res.send({status:false,msg:"error e",error: e.errmsg||e.message})
    } 
}
...
controlador.exist = async (req,res) =>{  
    try{  
        ...
        res.json({status:false,msg:"error e",error: e.errmsg||e.message})
    } 
}
...
controlador.profile = async (req,res) =>{
    try{
        ...
        res.json({status:!!user,msg:"ok",data:user})
    }catch(e){
        res.json({status:false,msg:"error e",error: e.errmsg||e.message})
    } 
}
